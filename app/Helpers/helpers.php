...
if (!function_exists('setting')) {
    ...
    function setting($key, $default = null)
    {
        ...
        return Cache::rememberForever("setting_{$group}_{$name}", function () use ($group, $name, $default) {
            $setting = Settings::where('group', $group)->where('name', $name)->first();
...
if (!function_exists('birthday_date')) {
    function birthday_date($date): string
        {
            ...
            Carbon::setLocale('es');
                  ...
                  return strtoupper($date->isoFormat('DD [de] MMMM'));
        }
    }
